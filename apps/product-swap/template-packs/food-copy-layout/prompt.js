...
function buildPrompt({
    ...
    const instructions = [
        ...
        '先在内部确定最终文案，优先使用常见、简短、无歧义的中文表达。',
        '生成前逐字核对最终文案，检查错别字、漏字、重复字、日期时间和标点；画面中只能出现核对后的文字。',
        '无法确认某个字时，改用更简单的同义表达或删除该句，不得使用形近字、乱码或拼音替代。',
        '使用清晰易读的常规字体，不使用可能导致字形误判的手写体、艺术字或变形文字。',
